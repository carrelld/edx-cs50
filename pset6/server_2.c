...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while ((bytesRead = fread(crawler, 1, bufferSize, file)) > 0)
    {
        *length += bytesRead;
        *content = realloc(*content, i++ * bufferSize + 1);
...
const char* lookup(const char* path)
{
    ...
    if (!ext || *(ext + 1) == '\0')
    {
        printf("ERR: no file extension in specified path\n");
        return NULL;
    }
    ext = ext + 1; //pointer arithmetic to trunc period
    
    
    // Case-by-case return values for each file extension
    // Could be done with a dictionary, and loop, but not enough for the trouble
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    for (int i = 1, j = 1, n = strlen(temp); i < n; i++)
    {
    ...
    // Validate HTTP-version and CRLF
    ...
    {
        // Effectively split request_target on '?' into strings, request_target
        // and queryPtr
        if (request_target[i] == '?')
        {
            request_target[i] = '\0';
            queryPtr = request_target + i + 1;
            break;
        }
    }
    
    // Set abs_path and query depending on wheter a query was present
    memcpy(abs_path, request_target, strlen(request_target) + 1);
    
    if (queryPtr)
    {
        memcpy(query, queryPtr, strlen(queryPtr) + 1);
